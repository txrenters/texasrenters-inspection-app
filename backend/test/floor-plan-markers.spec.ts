...
describe('floor-plan marker extraction schema', () => {
  it('accepts a valid normalized marker and bounding box', () => {
    const parsed = floorPlanExtractionSchema.parse([
      {
        ...baseArea,
        marker: { x: 0.54, y: 0.16, confidence: 0.9 },
        boundingBox: { x: 0.4, y: 0.02, width: 0.19, height: 0.27 },
      },
    ]);
  ...
  it('drops a marker with x below 0 or above 1 but keeps the area', () => {
    expect(
      floorPlanExtractionSchema.parse([{ ...baseArea, marker: { x: -0.1, y: 0.2 } }])[0].marker,
    ).toBeUndefined();
  ...
  it('drops a marker with y out of range but keeps the area', () => {
    expect(
      floorPlanExtractionSchema.parse([{ ...baseArea, marker: { x: 0.2, y: -0.3 } }])[0].marker,
    ).toBeUndefined();
    expect(
      floorPlanExtractionSchema.parse([{ ...baseArea, marker: { x: 0.2, y: 2 } }])[0].marker,
    ).toBeUndefined();
  });
...
describe('updateAreaMarker (spec §12/§18/§19)', () => {
  it('places a marker on a legacy area, binds it to the current plan, and audits — without touching status', async () => {
    ...
    const prisma = {
      propertyArea: {
        findFirst: jest
          .fn()
          .mockResolvedValue(areaRow({ markerX: null, markerY: null, sourceFloorPlanId: null })),
  ...
  it('rejects a stale marker edit instead of overwriting a newer revision', async () => {
    ...
    const prisma = {
      propertyArea: {
        findFirst: jest
          .fn()
          .mockResolvedValue(areaRow({ markerX: 0.1, markerY: 0.1, sourceFloorPlanId: 'plan-1' })),
      },
  ...
  it('backfills only missing markers and never rewrites names, order, or status', async () => {
    ...
    expect(prisma.auditLog.create).toHaveBeenCalledWith(
      expect.objectContaining({
        data: expect.objectContaining({ action: 'AREA_MARKER_GENERATED' }),
      }),
    );
  });
  ...
  it('exposes a mapped marker DTO (no raw coordinate columns)', async () => {
    ...
    const result = (await service(prisma).updateAreaMarker(admin, 'area-1', {
      x: 0.5,
      y: 0.25,
