 ...
 *
 * An unset tenant passes. That is deliberate and is what keeps the org-less
 * paths working — boot-time sweeps, the Cloudflare webhook, public report
 * links, password reset, and the query that resolves the organization in the
 * first place. It also means a request that never reaches the interceptor is
 * unrestricted, which is why this is a second wall and not the first.
 */
const UNSET = `current_setting('app.organization_id', true) is null
...
--
-- An unset app.organization_id passes every policy, which is what keeps the
-- org-less paths working: boot-time sweeps, the Cloudflare Stream webhook,
-- public homeowner report links, password reset, and the query that resolves
