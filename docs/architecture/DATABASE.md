...
The persistent backend uses one process-wide Prisma client. Runtime traffic uses Supabase's transaction pooler on port 6543 with Prisma PgBouncer mode; `DIRECT_URL` on port 5432 is reserved for migrations and administrative commands. Pooler URLs receive a default Prisma `connection_limit=5` and `pool_timeout=10`; explicit URL values take precedence. Deployments can configure these defaults with `DATABASE_CONNECTION_LIMIT` and `DATABASE_POOL_TIMEOUT_SECONDS` so all backend instances remain within the project connection budget.
