...
2. ✅ **Tenant GUC per request.** `AsyncLocalStorage` carries the organization;
   ...
   **Cost, measured on this database:** a scoped `findMany` is **4.29 ms**
   ...
   issuing a dozen queries.

   **`RLS_TENANT_SCOPE_ENABLED=false` relaxes the policies as well as stopping
   the per-query transaction.** Once the policies fail closed (3d), turning off
   only the application half would have been a kill switch rather than an escape
   hatch — every query would have carried no tenant and matched nothing. With it
   off, the connection carries `-c app.organization_id=*` instead: set once when
   the connection opens, so the policies pass and there is no per-query
   transaction to pay for. Leaking `'*'` across a pooled connection is harmless
   here and only here, because with scoping off there is no per-tenant value
   that could leak in its place.

   Verified both ways against the running stack: off, every admin endpoint
   still returns real data; on, enforcement is back.
3. ✅ **Policies.** 48 of 56 tables: 1 root, 25 direct, 22 transitive (deepest
   `BaselineMedia`, three hops). Generated by
   `scripts/generate-rls-policies.mjs` from the **live foreign-key graph**, not
   written by hand — 22 transitive paths transcribed manually is how a table
   quietly ends up unpoliced, and a missing policy is invisible.

   `DATABASE_URL` now connects as `texasrenters_app`, so the policies actually
   bind; `DIRECT_URL` stays the owner for migrations.

   **A bug the generator caught.** Its first run routed `PropertyArea` through
   `propertyware_units` rather than `Property` — both one hop, but `unitId` is
   **nullable**, so every building-level area would have failed its own `EXISTS`
   and vanished from its own tenant. Nullable foreign keys are no longer
   traversed at all: they cannot carry tenancy. `PropertyAreaAlias` inherited
   the same fault.

   **8 tables are deliberately unpoliced** and the generator reports them rather
   than skipping silently: `UserProfile`, `AuthCredential`, `AuthRefreshToken`,
   `AuthPasswordResetToken` and `MobilePushDevice` are user-scoped — a person
   can belong to several organizations — plus `WebhookEvent`,
   `propertyware_sync_locks` and `_prisma_migrations`, which have no
   organization dimension at all.

   Verified against the live database through the real `PrismaService`: reads
   scoped at every depth (correct tenant sees all, wrong tenant sees **0**),
   `WITH CHECK` refuses a row planted into another organization, and all the
   org-less paths still work. Then end to end through the running stack —
